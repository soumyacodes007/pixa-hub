...
export async function adminCreditHandler(c) {
    let body;
    try {
    ...
    try {
        ...
        if (!user)
            throw new UserNotFoundError(algorandAddress);
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            // Idempotency: unique constraint on algo_tx_id prevents double-credit
            const insertResult = await client.query(`INSERT INTO ledger_entries (user_id, amount_atomic, reason, algo_tx_id, ref_tx_id)
         VALUES ($1, $2, 'algorand_deposit', $3, $4)
         ON CONFLICT (algo_tx_id) DO NOTHING
         RETURNING id`, [user.id, amountAtomic.toString(), algoTxId, note ?? null]);
            if (insertResult.rows.length === 0) {
                await client.query('ROLLBACK');
                throw new DuplicateDepositError(algoTxId);
            }
            // Credit the balance
            await client.query(`UPDATE balances SET balance_atomic = balance_atomic + $1, updated_at = now()
         WHERE user_id = $2`, [amountAtomic.toString(), user.id]);
            // Fetch new balance for response
            const balResult = await client.query(`SELECT balance_atomic FROM balances WHERE user_id = $1`, [user.id]);
