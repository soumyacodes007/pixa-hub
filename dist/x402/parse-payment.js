 ...
 */
export function parsePaymentRequirements(raw) {
 ...
 */
export function resolveSellerNetwork(paymentRequirements) {
 ...
 */
export function getSellerAddress(paymentRequirements, network) {
    const accept = paymentRequirements.accepts.find((a) => normalizeNetwork(a.network) === normalizeNetwork(network) &&
        a.scheme === 'exact');
    if (!accept)
        throw new Error(`No accept entry for network ${network}`);
    return accept.payTo;
}
/**
 * Extracts the required payment amount in atomic units.
 */
export function getPaymentAmountAtomic(paymentRequirements, network) {
    const accept = paymentRequirements.accepts.find((a) => normalizeNetwork(a.network) === normalizeNetwork(network) &&
        a.scheme === 'exact');
    if (!accept)
        throw new Error(`No accept entry for network ${network}`);
    ...
    if (!amount) {
        throw new Error(`Accept entry for network ${network} is missing both 'amount' and 'maxAmountRequired'`);
    }
    return BigInt(amount);
}
//# sourceMappingURL=parse-payment.js.map