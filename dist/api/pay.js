...
import { parsePaymentRequirements, resolveSellerNetwork, getSellerAddress, getPaymentAmountAtomic, } from '../x402/parse-payment.js';
import { InsufficientBalanceError, UnsupportedNetworkError, toHttpException, } from '../core/errors.js';
// ─── Helper: get or create user + balance row ────────────────────────────────
async function getOrCreateUser(algorandAddr) {
    ...
    try {
        ...
        const userResult = await client.query(`INSERT INTO users (algorand_addr) VALUES ($1) 
       ON CONFLICT (algorand_addr) DO UPDATE SET algorand_addr = EXCLUDED.algorand_addr
       RETURNING id`, [algorandAddr]);
        const userId = userResult.rows[0].id;
        await client.query(`INSERT INTO balances (user_id, balance_atomic) VALUES ($1, 0)
...
// ─── Main handler ─────────────────────────────────────────────────────────────
export async function payHandler(c) {
    ...
    try {
        ...
        try {
            ...
            const insertResult = await client.query(`INSERT INTO payment_attempts (correlation_id, user_id, seller_network, seller_address, amount_atomic, status)
         VALUES ($1, $2, $3, $4, $5, 'pending')
         ON CONFLICT (correlation_id) DO NOTHING
         RETURNING id`, [correlationId, user.id, sellerNetwork, sellerAddress, amountAtomic.toString()]);
            if (insertResult.rows.length === 0) {
                // Row already exists — return existing result
                const existing = await client.query(`SELECT status, payment_sig, error_msg, seller_network 
           FROM payment_attempts WHERE correlation_id = $1`, [correlationId]);
                await client.query('COMMIT');
                ...
                if (row?.status === 'failed') {
                    return c.json({ success: false, error: row.error_msg ?? 'Payment failed' }, 402);
                }
                // Still 'pending' or 'paying' — rare race, let client retry
                return c.json({ success: false, error: 'Payment in progress, retry shortly' }, 202);
            }
            // We own the row — check and debit balance
            const balanceResult = await client.query(`SELECT balance_atomic FROM balances WHERE user_id = $1 FOR UPDATE`, [user.id]);
            ...
            if (currentBalance < amountAtomic) {
                ...
                await client.query(`DELETE FROM payment_attempts WHERE correlation_id = $1`, [correlationId]);
                await client.query('COMMIT');
                throw new InsufficientBalanceError(currentBalance, amountAtomic);
            }
            // Debit balance + mark as 'paying'
            await client.query(`UPDATE balances SET balance_atomic = balance_atomic - $1, updated_at = now() WHERE user_id = $2`, [amountAtomic.toString(), user.id]);
            await client.query(`UPDATE payment_attempts SET status = 'paying', updated_at = now() WHERE correlation_id = $1`, [correlationId]);
            await client.query('COMMIT');
        }
        catch (err) {
            await client.query('ROLLBACK');
        ...
        try {
            ...
            try {
                ...
                await rollbackClient.query(`UPDATE balances SET balance_atomic = balance_atomic + $1, updated_at = now() WHERE user_id = $2`, [amountAtomic.toString(), user.id]);
                await rollbackClient.query(`UPDATE payment_attempts SET status = 'failed', error_msg = $1, updated_at = now() WHERE correlation_id = $2`, [payErr.message, correlationId]);
        ...
        try {
            ...
            await settleClient.query(`UPDATE payment_attempts 
         SET status = 'paid', payment_sig = $1, updated_at = now() 
         WHERE correlation_id = $2`, [paymentSignature, correlationId]);
            await settleClient.query(`INSERT INTO ledger_entries (user_id, amount_atomic, reason, ref_tx_id)
         VALUES ($1, $2, 'base_payment', $3)`, [user.id, (-amountAtomic).toString(), correlationId]);
            await settleClient.query(`INSERT INTO treasury_records (chain, correlation_id, amount_atomic, direction)
         VALUES ('base', $1, $2, 'payment_out')`, [correlationId, amountAtomic.toString()]);
        ...
        return c.json({ success: false, error: httpErr.message }, httpErr.status);
    }
}
